...
As of 2026-09-25, GitHub `main` includes the restore activation fix merged in PR #5 (`ac40ac1`), but that fix is not deployed to Finland. The Finland checkout `/root/xui-backendhandler` remains locally clean on `main` at `9e77846`, and `/usr/local/bin/xui-backend` remains at `c19f8df8618dfc07c544f346af740c9875de7d3a`. `xui-backend.service` and `xui-backend-instance-kitten.service` are active, `/healthz` is OK, and the live database has migrations 001–014. The attempted code-only deployment stopped before mutation: `upgrade.sh` accepts only schema 012, while Finland is at schema 014, and the required off-host backup mount and protected dry-run env file `/root/xui-upgrade-dry-run.env` are absent. Do not use `upgrade.sh` on this host until its schema gate and these prerequisites are addressed.
