...
As of 2026-09-25, the Finland checkout and installed binary run merged `main` SHA `61774c56bb76a67d1d17ece1085f82b9477f7712`, including the restore activation fix from PR #5 and the bot admin fixes from PR #7. This was a code-only rollout with no migration or persistent-format changes. Both `xui-backend.service` and `xui-backend-instance-kitten.service` are active; `/healthz` and the instance-scoped admin API check passed. The database remains at migrations 001–014, including all four columns from migrations 013/014. The previous `c19f8df` binary is retained in protected root-owned rollback storage. `upgrade.sh` still accepts only schema 012; use it only after a schema-014-aware procedure and its off-host backup and rehearsal prerequisites are available.
