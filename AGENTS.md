...
- The agent-run production target is the registered Finland connection `mcp__finland_mcp_server`, connection `default`, currently authenticated as `root`. Use that managed connection for runtime work; never request or copy its credentials. As of 2026-09-25, checkout `/root/xui-backendhandler` is clean on local `main` at `9e77846`, tracking `origin/main`; `/usr/local/bin/xui-backend` remains at merged `main` revision `c19f8df` because later commits are docs-only, and the previous binary is retained in protected root-owned rollback storage. Active units are `xui-backend.service` and `xui-backend-instance-kitten.service`. The live database has the exact migrations 001–014 and all four columns from migrations 013/014. Reverify the checkout path, deployed SHA, active units, and schema on every deployment.
