...
- The agent-run production target is the registered Finland connection `mcp__finland_mcp_server`, connection `default`, currently authenticated as `root`. Use that managed connection for runtime work; never request or copy its credentials. After the 2026-09-25 rollout, checkout `/root/xui-backendhandler` remains at stale revision `d77d591`, while `/usr/local/bin/xui-backend` embeds merged `main` revision `c19f8df`; the previous binary is retained in protected root-owned rollback storage. Active units are `xui-backend.service` and `xui-backend-instance-kitten.service`. The live database has the exact migrations 001–014 and all four columns from migrations 013/014. Reverify the checkout path, deployed SHA, active units, and schema on every deployment.
