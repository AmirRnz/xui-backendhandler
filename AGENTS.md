...
- The agent-run production target is the registered Finland connection `mcp__finland_mcp_server`, connection `default`, currently authenticated as `root`. Use that managed connection for runtime work; never request or copy its credentials. At last inspection (2026-09-25), checkout `/root/xui-backendhandler` was at `d77d591` on the stale `feat/unified-backend-cli-installer-backups` branch, active units were `xui-backend.service` and `xui-backend-instance-kitten.service`, and the live database was at schema 014. Reverify the checkout path, deployed SHA, active units, and schema on every deployment.
...
- Before changing the VPS, review `docs/BACKUP.md` and the current deployment procedure. `upgrade.sh` supports only a schema-012 source, requires an off-host backup mount and a protected `DRY_RUN_DATABASE_URL`, and is not compatible with the live schema-014 host. Do not bypass those checks or claim that `upgrade.sh` deploys. Update and review the deployment procedure before live rollout. Stop and ask only when required external backup storage or access is genuinely unavailable.
