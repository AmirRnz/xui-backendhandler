...
- The agent-run production target is the registered Finland connection `mcp__finland_mcp_server`, connection `default`, currently authenticated as `root`. Use that managed connection for runtime work; never request or copy its credentials. At its last runtime sync on 2026-09-25, checkout `/root/xui-backendhandler` was locally clean on `main` at `61774c56bb76a67d1d17ece1085f82b9477f7712`, matching the installed binary's build revision. GitHub `main` has only documentation-only commits after that runtime SHA. The previous verified binary at `c19f8df` is retained in protected root-owned rollback storage. Active units are `xui-backend.service` and `xui-backend-instance-kitten.service`. The live database has migrations 001–014 and all four columns from migrations 013/014. The deployed bot unit, `/healthz`, and authenticated scoped admin access were verified after the runtime sync. Reverify the checkout path, deployed SHA, active units, and schema on every deployment.
