...
| `POST /v1/subscriptions/{id}/quotes` | `action` (`extend` or `upgrade_ip`), action-specific `months`/`ip_limit`, and `idempotency_key` | Stores an immutable account-owned renewal or device-cap quote; customer device caps stay in the `devices: N` comment, and mutations keep 3x-ui `limitIp=0` |
| `POST /v1/subscriptions/{id}/mutations` | `quote_id, payment_method, idempotency_key` | Debits the wallet atomically or creates a receipt-backed payment intent, then stores a durable full-row `subscription_update` for the backend worker; ownership and stale-quote checks run in the backend |
...
Only the backend worker calls 3x-ui. API token material comes from `XUI_PANEL_TOKENS_JSON`, keyed by panel ID. The worker requires version `3.8.5`; client updates read and preserve the full remote row, while enforcing the backend-owned `limitIp=0` policy. Immediate readback verifies that panel-owned fields, including secrets, survived the full-row update. If a process restart interrupts that verification, the work stays attempted and goes to manual review rather than repeating an ambiguous write. Adds/updates/attaches that are partial or ambiguous are read back before any retry. Provisioning obligations are committed before the external call. Retail device caps are stored as customer terms in the subscription record; new panel clients use `limitIp=0` and carry a legacy-compatible `devices: N` marker in the comment so older admin tooling can still read the configured cap.
